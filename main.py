...
async def main(pdf_path):
    ...
    # 3.根据简历信息初步筛选岗位
    print("⌛️开始初步筛选适合岗位...")
    filtBot = create_qiBot_from_config(type="filtBot")
    result = await filtBot.process_query(query=resume_json)
    await filtBot.close()

    from kb import DocumentQAClient
     # 配置信息
    APPId = "xxx"
    APISecret = "xxx"
    ...
    answer = client.get_jobs_info(result)
    print(answer)
    print("✅初步筛选岗位成功！")
    ...
    result = await matchingBot.process_query(query="简历信息：\n" + resume_json + "\n初步匹配岗位序列号列表：\n" + str(answer))
